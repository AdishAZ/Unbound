...
std::optional<Character> CharacterRepository::CreateCharacter(uint64_t account_id, const std::string& name, const std::string& appearance) {
    ...
    try {
        ...
        return db_pool_->ExecuteTransaction([&](pqxx::work& txn) -> std::optional<Character> {
            ...
            std::string_view empty_bin(reinterpret_cast<const char*>(empty_blob.data()), empty_blob.size());
...
std::optional<Character> CharacterRepository::LoadCharacter(uint64_t character_id) {
    try {
        return db_pool_->ExecuteTransaction([&](pqxx::work& txn) -> std::optional<Character> {
            ...
            c.x = row["x"].as<float>();
            c.y = row["y"].as<float>();
            c.direction = static_cast<uint8_t>(row["direction"].as<int>());
...
bool CharacterRepository::UpdateCharacter(const Character& character) {
    try {
        return db_pool_->ExecuteTransaction([&](pqxx::work& txn) {
            pqxx::result res = txn.exec_prepared("update_character", character.name, character.appearance, character.play_time_seconds, character.last_login, character.map_id, character.x, character.y, static_cast<int>(character.direction), character.money, character.id);
