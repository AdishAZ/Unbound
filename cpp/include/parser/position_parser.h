...
public:
    PositionParser(const memory::MemoryApi& memory, const memory::AddressTable& addresses);
    ParseResult<PlayerPosition> Parse(int64_t frame_count);

private:
    const memory::MemoryApi& memory_;
    const memory::AddressTable& addresses_;
    int64_t last_frame_ = -1;
    ParseResult<PlayerPosition> cached_result_;
};
