...
bool FontManager::Initialize(const std::string& default_fallback_path) {
    ...
    if (TTF_Init() == -1) {
        LOG_ERROR(UI, "Failed to initialize SDL_ttf: {}", TTF_GetError());
        return false;
    }

    default_fallback_path_ = default_fallback_path;
    initialized_ = true;
    LOG_INFO(UI, "[UI] SDL_ttf initialized");
...
void FontManager::Shutdown() {
    ...
    TTF_Quit();
    initialized_ = false;
    LOG_INFO(UI, "[UI] SDL_ttf shutdown complete");
}
...
TTF_Font* FontManager::LoadInternal(const std::string& path, int size) {
    ...
    if (font) {
        LOG_INFO(UI, "[UI] Loaded font: {}", path);
        LOG_INFO(UI, "[UI] Cached font: {} px", size);
        LOG_INFO(UI, "[UI] Loaded glyph count: {}", TTF_FontFaces(font));
    }
    return font;
}
...
TTF_Font* FontManager::GetFont(const std::string& font_path, int pt_size) {
    ...
    if (!font && !theme_fallback_path_.empty() && request_path != theme_fallback_path_) {
        LOG_INFO(UI, "[UI] Requested font missing: {}", request_path);
        LOG_INFO(UI, "[UI] Using fallback font: {}", theme_fallback_path_);
    ...
    if (!font && request_path != default_fallback_path_ && theme_fallback_path_ != default_fallback_path_) {
        LOG_INFO(UI, "[UI] Requested font missing");
        LOG_INFO(UI, "[UI] Using fallback font: {}", default_fallback_path_);
    ...
    if (font) {
        ...
        LOG_ERROR(UI, "[UI] Failed to load any fallback font for size {}", pt_size);
    }
