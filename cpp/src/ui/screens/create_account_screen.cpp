...
CreateAccountScreen::CreateAccountScreen(UIEngine* engine) : UIScreen("CreateAccountScreen"), engine_(engine) {
    ...
    submit_btn->OnClick([this]() {
        ...
        p.type = unboundmp::network::PacketType::kCreateAccountRequest;
        p.payload = auth_req.Serialize();
        client->SendPacket(p);
        
        state_ = CreateState::Requesting;
...
void CreateAccountScreen::Update(float dt) {
    ...
    if (state_ == CreateState::Connecting) {
        state_timer_ += dt;
        auto client = engine_->GetNetworkClient();
    ...
    if (state_ == CreateState::Requesting) {
        auto client = engine_->GetNetworkClient();
        if (client) {
