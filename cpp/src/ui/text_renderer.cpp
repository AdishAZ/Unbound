...
void TextRenderer::DrawText(const RenderContext& ctx, const std::string& text, const std::string& font_path, int size, int x, int y, const Color& color, Alignment align) {
    ...
    SDL_SetTextureAlphaMod(cached->texture, static_cast<Uint8>(color.a * ctx.alpha));
...
void TextRenderer::ClearCache() {
    ...
    LOG_INFO(UI, "[UI] Text cache hits: {}", cache_hits_);
    LOG_INFO(UI, "[UI] Text cache misses: {}", cache_misses_);
    cache_hits_ = 0;
    cache_misses_ = 0;
}
